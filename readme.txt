packages used:
express,
cors,
express,
pg,
pool,
nodemon


I had Created a simple crud opration for our application 
I had used nodejs(express) and postgresql as a database
I had installed packages like cors,nodemon,pool,pg,
post(for adding),put(for Editing),get(for getting all the data),delete(for deleting )
 has been done as per required task 

 NOTE:PUT AND DELETE WAS NOT MENTIONED